...
public class MatrizService {
    ...
    public int[][] toChannelMatrix(BiFunction<Integer, Integer, Double> canal, int ancho, int alto) {
        int[][] matrix = new int[ancho][alto];

        for (int x = 0; x < ancho; x++) {
            for (int y = 0; y < alto; y++) {
                matrix[x][y] = (int) (canal.apply(x, y) * 255);
            }
        }
        return matrix;
    }

    //SOLO PARA IMAGENES GRISES
    public int[][] aMatrizDeGrises(Image imagen) {
